...
// [C->S] vote:recast
export class VoteRecastPayload extends BasePayload {
  @IsString({ message: 'oldCandidateId는 문자열이어야 합니다' })
  @IsNotEmpty({ message: 'oldCandidateId는 비어있을 수 없습니다' })
  oldCandidateId: string

  @IsString({ message: 'newCandidateId는 문자열이어야 합니다' })
  @IsNotEmpty({ message: 'newCandidateId는 비어있을 수 없습니다' })
