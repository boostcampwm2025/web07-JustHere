...
describe('RoomRepository', () => {
  ...
  describe('createRoom', () => {
    it('방을 생성하고 slug를 포함한 Room 객체를 반환해야 한다', async () => {
      ...
      jest.spyOn(prisma.room, 'create').mockResolvedValue(mockRoom)
    ...
    it('slug 중복 시 재시도해야 한다', async () => {
      ...
      jest.spyOn(prisma.room, 'create').mockRejectedValueOnce(duplicateError).mockResolvedValueOnce(mockRoom)
    ...
    it('3회 재시도 후에도 실패하면 에러를 던져야 한다', async () => {
      ...
      jest.spyOn(prisma.room, 'create').mockRejectedValue(duplicateError)
    ...
    it('P2002가 아닌 다른 에러는 즉시 던져야 한다', async () => {
      ...
      jest.spyOn(prisma.room, 'create').mockRejectedValue(otherError)
