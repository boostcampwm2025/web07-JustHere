...
export const useMiddleLocation = () => {
  ...
  const findMiddleLocations = async (users: UserLocation[]) => {
    ...
    try {
      ...
      // POST 요청으로 중간 위치 찾기
      const response = await fetch('http://localhost:3000/api/odsay/find-middle-location', {
        method: 'POST',
        headers: {
      ...
      const errorMessage = err instanceof Error ? err.message : '중간 위치를 찾는 중 오류가 발생했습니다.';
