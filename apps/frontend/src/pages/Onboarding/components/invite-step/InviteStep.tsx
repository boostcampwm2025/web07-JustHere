...
import { ContentCopyIcon, CheckIcon, Button } from '@/shared/ui'
...
export const InviteStep = ({ selectedLocation, inviteLink, onComplete }: InviteStepProps) => {
  ...
  const truncatedLink = inviteLink.length > 35 ? `${inviteLink.slice(0, 35)}..` : inviteLink

  return (
    <>
      <h1 className="text-3xl font-bold text-black text-center mb-6">참여자 초대</h1>
      ...
      <div className="flex items-center gap-2 mb-6 max-w-md mx-auto">
        <div className="flex-1 h-12 px-4 bg-gray-bg border border-gray-300 rounded-xl flex items-center">
          <span className="text-base text-black truncate">{truncatedLink}</span>
        </div>
        <Button
          onClick={handleCopy}
          ...
          variant="ghost"
        >
          {copied ? <CheckIcon className="w-5 h-5 text-white" /> : <ContentCopyIcon className="w-5 h-5 text-white" />}
        </Button>
      </div>

      <Button onClick={onComplete} size="lg" className="w-full max-w-md mx-auto block h-13 py-4 text-base font-bold">
        시작하기
      </Button>
    </>
  )
