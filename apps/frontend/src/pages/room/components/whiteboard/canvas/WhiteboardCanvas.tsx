  ...
  const unifiedItems = useMemo<UnifiedCanvasItem[]>(() => {
    ...
    zIndexOrder.forEach(({ type, id }) => {
      if (type === 'line') {
        ...
        if (line) items.push({ type: 'line', data: line })
      } else if (type === 'postit') {
        const postIt = postItsMap.get(id)
        if (postIt) items.push({ type: 'postit', data: postIt })
      } else if (type === 'placeCard') {
        const card = placeCardsMap.get(id)
        if (card) items.push({ type: 'placeCard', data: card })
      } else if (type === 'textBox') {
        const textBox = textBoxesMap.get(id)
        if (textBox) items.push({ type: 'textBox', data: textBox })
      }
    })
