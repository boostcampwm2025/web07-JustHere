...
export default function PlaceDetailModal({ placeId, placeName, isOpen, onClose }: PlaceDetailModalProps) {
    ...
    <div className="fixed inset-0 z-60">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[480px] h-[85vh] max-h-[800px] bg-white z-40 shadow-xl rounded-3xl border border-gray-100 flex flex-col overflow-hidden">
        ...
        <div className="flex justify-between items-center p-4 border-b border-gray-100 shrink-0">
          <h3 className="text-lg font-bold text-gray-800 truncate pr-4">{placeName}</h3>
          <Button
            variant="ghost"
            size="icon"
            onClick={onClose}
        ...
        <div className="flex-1 w-full">
          <iframe src={placeUrl} title={`${placeName} 상세 정보`} className="w-full h-full border-0" allow="fullscreen" />
        </div>
      </div>
    </div>
  )
